...
- **Hallazgo nuevo (13-sep-2026) sobre por que `detect-price-drops` da 0 eventos hace meses: no es (solo) que no haya bajas de precio.** El 100% de las 1.386 propiedades activas con segment no-null tiene `segment = NULL` (verificado en la DB). La rama `good_deal` de la funcion exige `segment === 'refac'`, asi que jamas puede disparar mientras nadie llene esa columna. El ultimo evento de cualquier tipo (`opportunity_events`) es del 29-jun-2026, coincide con cuando algo dejo de setear `segment`. Sin diagnosticar aun donde se rompio ese seteo (scan, enrich, o una migracion).
