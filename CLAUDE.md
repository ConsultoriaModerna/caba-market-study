...
- **El mismo patron de verde-falso sigue vivo en tres puntos distintos (encontrado en la auditoria del 29-ago, sin arreglar todavia).** (1) `check-dead-listings.mjs` usa `fetch` proxeado plano contra ZP, que Cloudflare bloquea (a diferencia de scan/enrich, que usan puppeteer-stealth y si pasan): lleva noches seguidas con "200 checked, 0 dead, 200 unknown" y el resumen no distingue "0 bajas confirmadas" de "no pude confirmar ninguna". (2) El resumen del nightly reporta "ZP: 3582 new" cuando la DB solo registro 443 altas ese dia (probablemente suma coincidencias de texto en vez de leer el resultado real). (3) "Price drops" en el resumen queda fijo en 43 mientras el calculo real (`detect-price-drops`) da 0 varias noches seguidas. Los tres estan en `scripts/nightly-update.sh` / `scripts/vps/check-dead-listings.mjs`.
