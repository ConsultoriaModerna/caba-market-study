...
- **Pendiente de la misma auditoria, sin arreglar.** (1) `check-dead-listings.mjs` sigue usando `fetch` proxeado plano contra ZP, que Cloudflare bloquea (a diferencia de scan/enrich, que usan puppeteer-stealth y si pasan): sigue sin poder confirmar bajas reales, solo distingue "unknown" de "dead" pero la mayoria cae en unknown. Requiere reescribirlo con puppeteer-stealth, no es un fix chico.
